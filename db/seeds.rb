...
resp_epoch_earliest = Time.new(2013, 12, 11, 10, 0, 0).to_i
resp_epoch_latest = Time.new(2013, 12, 11, 11, 30, 0).to_i
dt = resp_epoch_latest - resp_epoch_earliest
rand_int = Random.new
...
5.times do |x|
  ...
  3.times do |y|
    q=l.questions.create!(text: "#{Faker::Lorem.sentence} Choose A, B, C, D", question_type: "MC")
    4.times do 
      Timecop.freeze(Time.at(rand_int.rand(resp_epoch_earliest..resp_epoch_latest)))
      r = q.responses.create!(value: ["A","B","C","D"].sample, correct: ["true", "false"].sample)
      # r= q.responses.create!(value: "A", correct: "true")
...
# q=l.questions.create(text: "Explain the relationship between the business cycle and tax revenue / government spending. Why does this present a challenge to policy makers? ", question_type: "SA")
